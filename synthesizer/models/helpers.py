...
class TacoTrainingHelper(Helper):
	...
	def next_inputs(self, time, outputs, state, sample_ids, name=None):
		with tf.name_scope(name or "TacoTrainingHelper"):
			...
			# self._ratio is initially 1
			# so to begin with, the next inputs are constantly set to be the targets at the previous timestep
			# then when the 29000 steps is reached, the ratio is decreased, meaning the outputs at the previous timestep will be gradually filtered in
			# the loss will probably increase as this happens
			next_inputs = tf.cond(
