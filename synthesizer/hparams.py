...
def get_image_list_2(data_root, speakers, holdout_phrases=None):
    filelist = []

    for speaker in speakers:
        for file_path in glob(os.path.join(data_root, f'{speaker}/*/*.jpg')):
            if holdout_phrases and file_path.split('/')[-2] in holdout_phrases:
                continue

            filelist.append(file_path)

    print('Filelist contains {} images'.format(len(filelist)))
...
hparams = HParams(
    ...
    tacotron_zoneout_rate=0.1,  # zoneout rate for all LSTM cells in the network (At each timestep, zoneout stochastically forces some hidden units to maintain their previous values)
    ...
    tacotron_teacher_forcing_decay_steps=62000,
    ...
    # eval_ckpt = 'synthesizer/saved_models/logs-pretrained/taco_pretrained/tacotron_model.ckpt-278000',
    eval_ckpt = 'synthesizer/saved_models/tacotron_model.ckpt-313000',
