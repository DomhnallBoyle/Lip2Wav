...
def main(args):
    ...
    if args.run_type == 'FROM_DATASET': 
        ...
        if args.path_replace: 
            video_paths = [p.replace(k, v) for p in video_paths for k, v in args.path_replace.items()]
...
if __name__ == '__main__':
    ...
    type_parser = parser.add_subparsers(dest='run_type')

    ###############################

    from_dataset_parser = type_parser.add_parser('FROM_DATASET')
    from_dataset_parser.add_argument('videos_root')

    dataset_type_parser = from_dataset_parser.add_subparsers(dest='dataset')

    parser_1 = dataset_type_parser.add_parser('GENERIC')
    ...
    parser_2 = dataset_type_parser.add_parser('SRAVI')
    ...
    parser_3 = dataset_type_parser.add_parser('GRID')
    parser_3.add_argument('--included_speakers', type=list_type)
    parser_3.add_argument('--excluded_speakers', type=list_type)

    parser_4 = dataset_type_parser.add_parser('LRS2')
    parser_4.add_argument('--videos_glob_path', help='*/*.mp4')
    parser_4.add_argument('--samples_text_file')

    ###############################

    from_file_parser = type_parser.add_parser('FROM_FILE')
    from_file_parser.add_argument('file_path')
    from_file_parser.add_argument('--path_replace', type=json.loads)

    args = parser.parse_args()
    assert args.run_type in ['FROM_DATASET', 'FROM_FILE']
